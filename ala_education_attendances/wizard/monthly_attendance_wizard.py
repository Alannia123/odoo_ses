...
class DailyAttendanceStudentRemark(models.TransientModel):
    ...
    academic_year_id = fields.Many2one(
        ...
        default=lambda obj: obj.env["ala.education.academic.year"].search([("enable", "=", True)]),
    )
    ...
    def generate_attendance(self):
        ...
        for rec in self:
            ...
            attendances = self.env['ala.education.attendance'].search([
                ('state', '=', 'done'),
        ...
        if not self.user_id:
            report_id = self.env.ref("ala_education_attendances.monthly_attendance_report")
